...
from estatistica.filters import PerguntaFilter
...
from questionarios.models import EstadoQuestionario, Questionario,Resposta, Pergunta

def estatistica(request):
    return render(request,
                  template_name="estatistica.html")

def estatistica_eficiencia(request):
    perguntas_filter = PerguntaFilter(request.GET, queryset=Pergunta.objects.filter(tema_id=7))

    chartdata_list = []
    for pergunta in perguntas_filter.qs:
        if pergunta.tipo_pergunta_id == 4:
            response_data = Resposta.objects.filter(pergunta=pergunta).values('conteudo_int').annotate(total=Count('conteudo_int')).order_by('-total')
            xdata = [resp['conteudo_int'] for resp in response_data if resp['conteudo_int'] is not None]
        elif pergunta.tipo_pergunta_id == 1 or pergunta.tipo_pergunta_id == 2 or pergunta.tipo_pergunta_id == 3:
            response_data = Resposta.objects.filter(pergunta=pergunta).values('conteudo_string').annotate(total=Count('conteudo_string')).order_by('-total')
            xdata = [resp['conteudo_string'] for resp in response_data if resp['conteudo_string'] is not None]

        ydata = [resp['total'] for resp in response_data if resp.get('conteudo_int') is not None or resp.get('conteudo_string') is not None]
        chartdata = {
            'question': pergunta.conteudo,
            'total_response': sum(ydata),
            'x': json.dumps(xdata),
            'y1': json.dumps(ydata),
            'extra1': {"tooltip": {"y_start": "", "y_end": " occurrences"}}
        }
        chartdata_list.append(chartdata)
        
    return render(request, "estatistica_eficiencia.html", {'chartdata_list': chartdata_list, 'filter': perguntas_filter})

def estatistica_funcionario(request):
    perguntas_filter = PerguntaFilter(request.GET, queryset=Pergunta.objects.filter(tema_id=5))
    ...
    return render(request, "estatistica_funcionario.html", {'chartdata_list': chartdata_list, 'filter': perguntas_filter})

def estatistica_questionario_tema(request):
    tema_counts = Questionario.objects.values('tema_id__titulo').annotate(questionario_count=Count('id')).order_by('tema_id')
    labels = [tema['tema_id__titulo'] for tema in tema_counts]
    data_points = [tema['questionario_count'] for tema in tema_counts]
    
    context = {
        'labels': labels,
        'data_points': data_points,
    }
    return render(request, "estatistica_questionario_tema.html", context)

def estatistica_satisfacao_utilizadores(request):
    perguntas_filter = PerguntaFilter(request.GET, queryset=Pergunta.objects.filter(tema_id=6))
    '''conteudo_counts = Resposta.objects.filter(conteudo_int__isnull=False).values('conteudo_int').annotate(total_respostas=Count('id')).order_by('conteudo_int')
    queryset = Questionario.objects.all()
    
    # Extrair rótulos e dados
    labels = [str(conteudo['conteudo_int']) for conteudo in conteudo_counts]
    data_points = [conteudo['total_respostas'] for conteudo in conteudo_counts]
    
    # Filtragem por ano
    ano = request.GET.get('ano')
    if ano:
        queryset = queryset.filter(anoletivoid__anoletivoid=ano)
    
    questionarios_filter = QuestionarioFilter(request.GET, queryset)
    ...
    return render(request, "estatistica_satisfacao_utilizadores.html", {'chartdata_list': chartdata_list, 'filter': perguntas_filter})
