...
####################################################################################################################
################### Inicio Pedido Parecer                 ####################################
####################################################################################################################
def criar_pedidos_parecer(request):
    erros = ""
    msg = ""
    user = request.user  # Recupera o usuário do request

    # Verifica se o usuário pertence ao grupo "Docente"
    if user.groups.filter(name="Docente").exists():
        if request.method == 'POST':
            subpedidos = json.loads(request.POST.get('subpedidos', '[]'))
            form = PedidoParecerForm(request.POST)
            descricao = request.POST.get('descricao', '')
            assunto = request.POST.get('assunto', '')
            data = request.POST.get('data', '')
            opcao = request.POST.get('selected_option')
            ...
            try:
                ...
                data_destino = ano_letivo.objects.get(ativo="S").dia_fim
                if data < hoje or data > data_destino:
                    erros = f"A data deve estar entre hoje e o fim do ano letivo ({data_destino})"
                    msg = True
                    return render(request, "pedido/criar_pedido_parecer.html", {'erros': erros, 'msg': msg, 'form': form})
            except ValueError:
                erros = "Data inválida."
                msg = True
                return render(request, "pedido/criar_pedido_parecer.html", {'erros': erros, 'msg': msg, 'form': form})

            # Validação dos subpedidos ou arquivo
            if opcao == "1":
                if not subpedidos:
                    erros = "Têm que haver pelo menos um subpedido"
                    msg = True
                ...
                if not arquivo or not str(arquivo).endswith(".xls"):
                    erros = "Ficheiro deve ser do tipo .xls"
                    msg = True
                    return render(request, "pedido/criar_pedido_parecer.html", {'erros': erros, 'msg': msg, 'form': form})

            # Validação do assunto e descrição
            if len(assunto) < 5:
                erros = "Assunto deve ter mais que 5 caracteres."
                msg = True
                return render(request, "pedido/criar_pedido_parecer.html", {'erros': erros, 'msg': msg, 'form': form})

            if len(descricao) < 5:
                erros = "Descrição deve ter mais que 5 caracteres."
                msg = True
                return render(request, "pedido/criar_pedido_parecer.html", {'erros': erros, 'msg': msg, 'form': form})
            ...
            if form.is_valid():
                try:
                    PedidoParecer.objects.get(docenteutilizadorid=request.user.id, data_de_alvo=data, assunto=assunto)
                    erros = "Já existe um pedido com esta data e assunto."
                    msg = True
                    return render(request, "pedido/criar_pedido_parecer.html", {'erros': erros, 'msg': msg, 'form': form})
                except ObjectDoesNotExist:
                    ...
                    pedido.data_de_submissao = datetime.date.today()
                    pedido.data_de_alvo = data
                    pedido.tipo = TipoDePedido.objects.get(id=5)  # Ajustar conforme necessário
                    pedido.assunto = assunto
                    pedido.anoletivoid = ano_letivo.objects.get(ativo="S")
                    pedido.informacoes = descricao
                    pedido.identificador_id = IdentificadorPedido.objects.get(id=opcao)
                    ...
                    # Salvando o arquivo se a opção 2 for escolhida
                    if opcao == "2" and arquivo:
                        ficheiros_dir = os.path.join(settings.MEDIA_ROOT, 'ficheiros')
                        if not os.path.exists(ficheiros_dir):
                            os.makedirs(ficheiros_dir)
                        ficheiro_nome = f'pedido_{pedido.id}.{arquivo.name.split(".")[-1]}'
                        with open(os.path.join(ficheiros_dir, ficheiro_nome), 'wb') as f:
                            for chunk in arquivo.chunks():
                                f.write(chunk)

                    # Salvando os subpedidos se a opção 1 for escolhida
                    elif opcao == "1":
                        for subpedido_data in subpedidos:
                            subpedido = SubpedidoParecer(
                                pedido_ptr_id=pedido.id,
                                assunto=subpedido_data['assunto'],
                                descricao=subpedido_data['descricao'],
                                nome_investigador=subpedido_data['nome_investigador']
                            )
                            subpedido.save()
                    return redirect('main:mensagem', 4)
        else:
            form = PedidoParecerForm()
...
def editar_pedidos_parecer(request, id):
    ...
    try:
        pedido_parecer = PedidoParecer.objects.get(id=id)
    except PedidoParecer.DoesNotExist:
        return redirect('main:editar-pedido-parecer')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if data < hoje or data > data_destino:
                erros = f"A data deve estar entre hoje e o fim do ano letivo ({data_destino})"
                msg = True
                return render(request, "pedido/editar_pedido_parecer.html", {'form': form, 'erros': erros, 'msg': msg, 'pedidos':subpedidos})
        except ValueError:
            erros = "Data inválida."
            msg = True
            return render(request, "pedido/editar_pedido_parecer.html", {'form': form, 'erros': erros, 'msg': msg, 'pedidos':subpedidos})
        ...
        # Salvando o formulário
        if form.is_valid():
            pedido = PedidoParecer.objects.get(id=id)
            pedido.data_de_alvo = data
            pedido.tipo = TipoDePedido.objects.get(id=5)
            pedido.assunto = assunto
            pedido.informacoes = descricao
            
            SubpedidoParecer.objects.filter(pedido_ptr_id=id).delete()  # Clear existing subpedidos
            for subpedido_data in subpedidos:
                subpedido = SubpedidoParecer(
                    pedido_ptr_id=pedido.id,
                    assunto=subpedido_data['assunto'],
        ...
        file_url = ""
        if identificador == 1:
            outros = SubpedidoParecer.objects.filter(pedido_ptr_id=id)
            for outro in outros:
                pedido_ficheiro_nome = "pedido_" + str(pedido.id) + "_" + str(outro.id) + ".pdf"
                file_url = os.path.join(settings.MEDIA_URL, pedido_ficheiro_nome)  # This is correct for the URL
                file_path = os.path.join(settings.MEDIA_ROOT, pedido_ficheiro_nome)  # This is the file system path
                    
                subpedido = {}
                subpedido['assunto'] = outro.assunto
                subpedido['descricao'] = outro.descricao
                subpedido['nome_investigador'] = outro.nome_investigador
                subpedido['file_exists'] = os.path.exists(file_path)  # Check the file system path
                subpedido['file_url'] = file_url  # Use the URL for linking in the template
...
def consultar_pedido_parecer(request, id):
    pedido = Pedido.objects.get(id=id)
    pedido.estado = pedido.estado_0.estado
    pedido.data_de_alvo = pedido.data_de_alvo.strftime('%d/%m/%Y')
    file_url = ''
    ...
    if pedido.identificador_id.id == 1:
        pareceres = SubpedidoParecer.objects.filter(pedido_ptr_id=pedido.id)
        subpedidos = []
        for parecer in pareceres:
            pedido_ficheiro_nome = "pedido_" + str(pedido.id) + "_" + str(parecer.id) + ".pdf"
            file_url = os.path.join(settings.MEDIA_URL, pedido_ficheiro_nome)  # updated to use MEDIA_URL
            

            subpedido = {}
            subpedido['assunto'] = parecer.assunto
            subpedido['descricao'] = parecer.descricao
            subpedido['nome_investigador'] = parecer.nome_investigador
            subpedido['ficheiro'] = parecer.ficheiro
            subpedido['file_url'] = file_url
            subpedidos.append(subpedido)
        pedido.subpedidos = subpedidos

    else:
        pedido_ficheiro_nome = "pedido_" + str(pedido.id) + ".pdf"
        file_url = os.path.join(settings.BASE_DIR, 'ficheiros', pedido_ficheiro_nome)     
        pedido.fire_url = file_url
    return render(request, 
                        'pedido/consultar_pedido_parecer.html',
                        context={'pedido_parecer': pedido, 'subpedidos': subpedidos,'file_url': file_url })
 
    
def eliminar_pedido_parecer(request, id):
    pedido = Pedido.objects.get(id=id)
    user = get_user(request)
        ...
        if pedido.estado_0.estado == 'Pendente':
            if request.method == 'POST':
                if 'confirmar' in request.POST:
                    subpedidos = []
                    if pedido.identificador_id.id == 1:
                        subpedidos = SubpedidoParecer.objects.filter(pedido_ptr_id=id)
                    
                    for subpedido in subpedidos:
                        pedido_ficheiro_nome = f'pedido_{pedido.id}_{subpedido.id}.pdf'
                        print(pedido_ficheiro_nome)
                        file_path = os.path.join(settings.MEDIA_ROOT, pedido_ficheiro_nome)
...
####################################################################################################################
################### Inicio           de criar Pedido ALUNO                 ########################################
####################################################################################################################
def criar_pedidos_aluno(request):
    erros = ""
    msg = ""
    tipo_subpedidos_json = json.dumps(list(TipoSubpedidoAluno.objects.values('id', 'nome')))
    if request.method == 'POST':
        form = PedidoDeAlunoForm(request.POST)
        informacoes = request.POST.get('informacoes', '')
        assunto = request.POST.get('assunto', '')
        opcao = request.POST.get('selected_option')
        data = request.POST.get('data', '') 
        numero_aluno = request.POST.get('aluno_id', '') 
        subpedidos = json.loads(request.POST['subpedidos'])
        
        try:
            aluno = Aluno.objects.get(numero_aluno=numero_aluno)
        except Aluno.DoesNotExist:
            erros = "O número do aluno não existe na base de dados."
            msg = True
            return render(request, "pedido/criar_pedido_aluno.html", {
                'erros': erros, 'msg': msg, 'form': form, 'pedidos': subpedidos
            })
        ...
        if form.is_valid():
            ...
            pedido.assunto = assunto
            pedido.anoletivoid = ano_letivo.objects.get(ativo="S")
            pedido.informacoes = informacoes
            pedido.aluno_id = numero_aluno
            pedido.save()  
            if opcao == "2":
                ficheiros_dir = os.path.join(settings.BASE_DIR, 'ficheiros')
                ficheiro_nome = f'pedido_{pedido.id}.{arquivo.name.split(".")[-1]}'
                ...
                for subpedido in subpedidos:
                    ...
                    tipo_subpedido_id = subpedido.get('tipoSubpedidoId')
                    tipo_subpedido_aluno = get_object_or_404(TipoSubpedidoAluno, pk=tipo_subpedido_id)

                    outros = SubPedidoAluno(assunto=subpedido['assunto'],
                                            descricao=subpedido['descricao'],
                                            pedido_aluno=pedido,
                                            tipo_subpedido_aluno=tipo_subpedido_aluno)
                    outros.save()            
            return redirect('main:mensagem', 4)
    else:
        form = PedidoDeAlunoForm()

    return render(request, 'pedido/criar_pedido_aluno.html', {'form': form, 'tipo_subpedidos':tipo_subpedidos_json})



####################################################################################################################
################### Fim de criar Pedido ALUNO                 ####################################3333
####################################################################################################################

####################################################################################################################
################### Inicio de Editar Pedido ALUNO                 ####################################3333
####################################################################################################################
def editar_pedidos_aluno(request,id):
    erros = ""
    aluno_name = ""
    identificador = ""
    file_url = ""
    aluno_numero = ""
    tipo_subpedidos_json = json.dumps(list(TipoSubpedidoAluno.objects.values('id', 'nome')))
    pedido_aluno = PedidoDeAluno.objects.select_related('aluno').get(pk=id)
    aluno_name = pedido_aluno.aluno.name
    aluno_numero = pedido_aluno.aluno.numero_aluno
    if request.method == 'POST':
        subpedidos = json.loads(request.POST['subpedidos'])
        form = PedidoDeAlunoForm(request.POST)
        descricao = request.POST.get('informacoes', '')
        ...
        try:
            ...
            if data < hoje or data > data_destino:
                ...
                msg = True
                return render(request, 
                    template_name="pedido/editar_pedido_aluno.html",
                    context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos, 'aluno_name': aluno_name,'aluno_numero': aluno_numero,'tipo_subpedidos':tipo_subpedidos_json})
        except ValueError:
            erros = "Data inválida."
            msg = True
            return render(request, 
        ...
        if len(assunto) < 5:
            ...
            return render(request, 
                  ...
                  context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos, 'aluno_name': aluno_name,'aluno_numero': aluno_numero, 'tipo_subpedidos':tipo_subpedidos_json})
        if form.is_valid():
            pedido = PedidoDeAluno.objects.get(id=id) 
            pedido.data_de_submissao = datetime.date.today()
            pedido.data_de_alvo = data
            pedido.tipo = TipoDePedido.objects.get(id=6)
            pedido.assunto = assunto
            ...
            numCount = len(subpedidos)
            if numCount == 0:
                erros = "Pedido tem que ter pelo menos um subpedido."
                msg = True
                return render(request, 
                  template_name="pedido/editar_pedido_aluno.html",
                  context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos, 'aluno_name': aluno_name,'aluno_numero': aluno_numero, 'tipo_subpedidos':tipo_subpedidos_json})
            ...
            SubPedidoAluno.objects.filter(pedido_aluno_id=id).delete()
            for subpedido in subpedidos:
                    try:
                        tipo_obj = TipoSubpedidoAluno.objects.get(nome=subpedido['tipo'])
                        tipo_id = tipo_obj.id
                    except ObjectDoesNotExist:
                        # Handle the case where no entry for the given 'tipo' exists
                        print("Tipo not found for:", subpedido['tipo'])
                        continue

                    outros = SubPedidoAluno(assunto=subpedido['assunto'],
                                    descricao=subpedido['descricao'],
                                    tipo_subpedido_aluno_id=tipo_id,
                                    pedido_aluno_id=pedido.id)
                    outros.save()  # Save the 'outros' instance to get a valid ID
                
                    pedido_ficheiro_nome = "pedido_" + str(pedido.id) + "_" + str(outros.id-numCount) + ".pdf"
                    file_path = os.path.join(settings.MEDIA_ROOT, pedido_ficheiro_nome) 
                    
                    if os.path.exists(file_path):
                        new_pedido_ficheiro_nome = "pedido_" + str(pedido.id) + "_" + str(outros.id) + ".pdf"
                        new_file_path = os.path.join(settings.MEDIA_ROOT, new_pedido_ficheiro_nome)
                        os.rename(file_path, new_file_path)  # Rename the file
                        subpedido['file_url'] = os.path.join(settings.MEDIA_URL, new_pedido_ficheiro_nome)
        ...
        aluno_numero = pedido_aluno.aluno.numero_aluno
        if identificador == 1:
            outros = SubPedidoAluno.objects.filter(pedido_aluno_id=id)
            outros_counter = outros.count()
            for outro in outros:
                pedido_ficheiro_nome = "pedido_" + str(pedido.id) + "_" + str(outro.id) + ".pdf"
                file_url = os.path.join(settings.MEDIA_URL, pedido_ficheiro_nome)  # This is correct for the URL
                file_path = os.path.join(settings.MEDIA_ROOT, pedido_ficheiro_nome)  # This is the file system path
                ...
                subpedido['assunto'] = outro.assunto
                subpedido['descricao'] = outro.descricao
                subpedido['tipo'] = outro.tipo_subpedido_aluno.nome
                subpedido['file_exists'] = os.path.exists(file_path)  # Check the file system path
                subpedido['file_url'] = file_url  # Use the URL for linking in the template
    ...
    return render(request, 
                    template_name="pedido/editar_pedido_aluno.html",
                    context={'tipo_subpedidos':tipo_subpedidos_json, 'aluno_name': aluno_name,'aluno_numero': aluno_numero, 'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos,'tipo': identificador,'file_url':file_url})

####################################################################################################################
################### Fim de Editar Pedido ALUNO                 ####################################3333
####################################################################################################################


####################################################################################################################
################### Incio de Consultar Pedido ALUNO                 ####################################3333
####################################################################################################################

def consultar_pedido_aluno(request,id):
    pedido = Pedido.objects.get(id=id);
    pedido.estado = pedido.estado_0.estado
    pedido.data_de_alvo = pedido.data_de_alvo.strftime('%d/%m/%Y');
    file_url = ""
    if pedido.identificador_id.id == 1:
        outros = SubPedidoAluno.objects.filter(pedido_aluno=pedido.id)
        subpedidos = []
        for outro in outros:
            pedido_ficheiro_nome = "pedido_" + str(pedido.id) + "_" + str(outro.id) + ".pdf"
            file_url = os.path.join(settings.MEDIA_URL, pedido_ficheiro_nome)
        
            subpedido = {}
            subpedido['assunto'] = outro.assunto
            subpedido['descricao'] = outro.descricao
            subpedido['tipo'] = outro.tipo_subpedido_aluno.nome
            subpedido['file_exists'] = os.path.exists(os.path.join(settings.MEDIA_ROOT, pedido_ficheiro_nome))
            subpedido['file_url'] = file_url
            subpedidos.append(subpedido)
        pedido.subpedidos = subpedidos
    else:
    ...
    return render(request, 
                        ...
                        context={'pedido_aluno': pedido, 'file_url': file_url, 'aluno_name': aluno_name, 'aluno_numero': aluno_numero })

####################################################################################################################
################### Fim de Consultar Pedido ALUNO                              ####################################
#####################################################################################################################


####################################################################################################################
################### Fim de Eliminar Pedido ALUNO                 ####################################3333
####################################################################################################################


def eliminar_pedido_aluno(request, id):
    pedido = Pedido.objects.get(id=id)
    user = get_user(request)
    if user.groups.filter(name = "Docente").exists():
        if pedido.estado_0.estado == 'Pendente':
            if request.method == 'POST':
                if 'confirmar' in request.POST:
                    if pedido.identificador_id.id == 1:
                        pedido_de_aluno = get_object_or_404(PedidoDeAluno, pedido_ptr_id=id)
                        subpedidos = SubPedidoAluno.objects.filter(pedido_aluno=pedido_de_aluno)
                        for subpedido in subpedidos:
                            pedido_ficheiro_nome = f"pedido_{pedido.id}_{subpedido.id}.pdf"
                            file_path = os.path.join(settings.MEDIA_ROOT, pedido_ficheiro_nome)
                            if os.path.exists(file_path):
