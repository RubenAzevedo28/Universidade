...
class PedidoParecerForm(forms.ModelForm):
    descricao = forms.CharField(max_length=500)
    data = forms.CharField(max_length=100)
    assunto = forms.CharField(max_length=100)

    class Meta:
        model = PedidoParecer  # Certifique-se de que está referenciando o modelo correto
        fields = ('descricao','data','assunto',)
...
class PedidoDeAlunoForm(forms.ModelForm):
    assunto = forms.CharField(max_length=255)
    aluno_id = forms.CharField(max_length=255)
    data = forms.DateField(widget=forms.DateInput(attrs={'type': 'date'}))
    informacoes = forms.CharField(widget=forms.Textarea, max_length=500)

    class Meta:
        model = PedidoDeAluno
        fields = ('data', 'assunto', 'informacoes', 'aluno_id')
