...
class Pedido(models.Model):
    ...
    def __str__(self):
        return f"Pedido(id={self.id}, estado={self.estado_0.estado if self.estado_0 else 'N/A'}, data_de_alvo={self.data_de_alvo})"
...
class PedidoParecer(Pedido):

    class Meta:
        db_table = 'pedido_parecer'

class SubpedidoParecer(models.Model):
    id = models.AutoField(db_column='ID', primary_key=True)  # Field name made lowercase.
    assunto = models.CharField(db_column='assunto', max_length=255, null=True)
    descricao = models.CharField(db_column='descricao', max_length=255, null=True)
    nome_investigador = models.CharField(db_column='nome_investigador', max_length=255, null=True)
    pedido_ptr = models.ForeignKey(PedidoParecer, on_delete=models.CASCADE, null=True)
    ficheiro = models.FileField(upload_to='subpedidos/', blank=True, null=True)

    class Meta:
        db_table = 'subpedido_parecer'
...
class Aluno(models.Model):
    numero_aluno = models.IntegerField(primary_key=True)
    name = models.CharField(max_length=255, blank=True, null=True)

    class Meta:
        db_table = 'aluno'

class TipoSubpedidoAluno(models.Model):
    nome = models.CharField(max_length=255)

    class Meta:
        db_table = 'tipo_subpedido_aluno'
...
class SubPedidoAluno(models.Model):
    assunto = models.CharField(max_length=255, default='Default Assunto')
    descricao = models.CharField(max_length=255, null=True)
    pedido_aluno = models.ForeignKey(PedidoDeAluno, on_delete=models.CASCADE, db_column='pedido_ptr_id', null=True)
    tipo_subpedido_aluno = models.ForeignKey(TipoSubpedidoAluno, on_delete=models.RESTRICT, db_column='tipo_subpedido_aluno_id')

    class Meta:
        db_table = 'subpedido_aluno'

    def __str__(self):
        return f"SubPedido {self.assunto} for Pedido {self.pedido_aluno.pedido_ptr}"